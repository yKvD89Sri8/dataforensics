...
def save_steps_loss_trajectory_batch(dataset, model_pipeline,data_file_path, img_process_transform, save_frequent=20, batch_size=2, guidance_scale=7.5):
...
def create_argparser():
    defaults = dict(
        ...
        save_frequency=1,
        batch_size=2, 
        data_file_path="../data/laion2b-en-data/00002.tar", 
        data_save_file_path="./model_v1_4_member_debug_laion2b_en.joblib",
...
pd_data = wds.WebDataset(data_file_path).shuffle(100000)

pipe = CustomizedDiffusionPipeline.from_pretrained(model_name)

pipe.to(device)

pipe.set_progress_bar_config(disable=args.verbose)

pipe.set_generated_sample_resolution(resolution)

extracted_ds = extract_datapoints_webdataset_member(pd_data, num_of_data, image_transforms)
logger.log("finish data extraction with {} samples".format(len(extracted_ds)))
save_steps_loss_trajectory_batch(extracted_ds, pipe,data_save_file_path, image_transforms, save_frequency, batch_size, guidance_scale)
logger.log("finish")
